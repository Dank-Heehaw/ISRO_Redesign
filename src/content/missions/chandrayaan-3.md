---
name: Chandrayaan-3
shortName: CH-3
category: planetary
status: completed
launchDate: 2023-07-14
vehicle: LVM3-M4
destination: Lunar South Polar Region
summary: >-
  The soft landing that made India the fourth nation to reach the lunar surface,
  and the first to touch down near the Moon's south pole.
accent: '#c9a227'
featured: true
order: 1
stats:
  - label: Landing
    value: 23 Aug 2023
  - label: Site
    value: Shiv Shakti Point
  - label: Rover range
    value: 101 m
  - label: Surface ops
    value: 14 Earth days
---

Chandrayaan-3 set the Vikram lander down at 69.37°S on 23 August 2023, in terrain
that no mission had previously attempted. The south polar highlands are of
interest precisely because they are difficult: permanently shadowed craters there
may hold water ice preserved for billions of years.

The Pragyan rover covered 101 metres over one lunar day. Its Alpha Particle X-ray
Spectrometer measured a composition at the landing site that ties the region to
ALHA 81005, the first lunar meteorite recovered on Earth — an unexpectedly direct
link between a rock in a laboratory and the ground it came from.

The lander's RAMBHA-LP instrument returned the first in-situ measurements of the
near-surface lunar plasma at these latitudes, and a commanded "hop" experiment
lifted Vikram roughly 40 cm and set it back down, demonstrating the thrust
control that a future sample-return ascent would require.
